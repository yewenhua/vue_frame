                ...
                <div v-for="(detail, index) in v" :key="index">
                    <section v-for="(item, kk) in detail" :key="kk">
                        ...
                        <aside>
                            ...
                            <p class="brief"><span :class="item.deviceOpenType == 'FINGER' ? 'text-finger' : item.deviceOpenType == 'PWD' ? 'text-pwd' : item.deviceOpenType == 'CARD' ? 'text-card' : item.deviceOpenType == 'REMOTE' ? 'text-remote' : item.deviceOpenType == 'LOCK' ? 'text-lock' : 'text-finger'">{{item.deviceOpenType == 'BLUE' || item.deviceOpenType == 'WIFI' ? (item.deviceOpenType == 'BLUE' ? '蓝牙' : 'WIFI') + '开锁 （ID:' + item.no + '）' : '--'}}</span></p>
                        </aside>
                    </section>
                </div>
            </article>
    ...
    export default {
        ...
        methods: {
            ...
            getData(){
                if(!this.isLoading) {
                    this.isLoading = true;
                    var token = this.token ? this.token : 'ca22c53d93102cf27b42ac7b8a399e45';
                    var url = this.axios.defaults.baseURL + '/deviceOpenRecord/list/ff8080815e04208c015e04f31b7a0000/253453c4a18b11e7abc4cec278b6b50a?access_token=' + token;
                    this.axios({
                        method: 'post',
                        url: url,
                        data: {
